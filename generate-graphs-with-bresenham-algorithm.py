...
save_path = "./graphs-bresenham"  # グラフ表示画像の保存先
...
def draw_point(canvas, x: int, y: int, width: int):
    """
    点を描画します
            ...
            for j in range(-width, width + 1):
                ...
                if 0 <= xi < size:
                    ...
                    if r <= width:  # 半径内部かどうかを判定
                        val = int(256 * (1 - r / width) - 1)  # 直線的に色を変化
                        # val = int((256 / (r_0 + 1)) - 1)  # 中心から反比例するように色を変化

                        if val > canvas[yi, xi]:  # 画素よりも大きい値であれば上書き
                            canvas[yi, xi] = val


def drawline_with_bresenham_algorithm(canvas, x0: int, y0: int, x1: int, y1: int, width: int):
    """
...
def generate_graph(seq, accession_number, size, padding, width):
    ...
    for i in range(len(mapped_points) - 1):
        ...
        drawline_with_bresenham_algorithm(canvas, start["x"], start["y"], end["x"], end["y"], width)
